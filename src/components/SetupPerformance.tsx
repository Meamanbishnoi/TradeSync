...
export default function SetupPerformance({ trades }: { trades: Trade[] }) {
  const setups = useMemo((): SetupStats[] => {
    const map: Record<string, { pnls: number[]; directions: string[] }> = {};

    for (const t of trades) {
      const key = t.setup?.trim() || "No Setup";
      if (!map[key]) map[key] = { pnls: [], directions: [] };
      map[key].pnls.push(t.pnl);
      map[key].directions.push(t.direction);
    }

    return Object.entries(map).map(([setup, { pnls, directions }]) => {
  ...
  const maxAbs = Math.max(...setups.map(s => Math.abs(s.netPnl)), 1);
  const fmtPnl = (v: number) => `${v >= 0 ? "+" : ""}$${Math.abs(v) >= 1000 ? `${(v / 1000).toFixed(1)}k` : Math.abs(v).toFixed(0)}`;
  ...
  return (
    <div>
      ...
      <>
        {/* Table — hidden on very small screens via CSS class */}
        <div className="setup-table-wrapper" style={{ overflowX: "auto", border: "1px solid var(--border-color)", borderRadius: "10px" }}>
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "13px" }}>
            <thead>
              <tr style={{ backgroundColor: "var(--bg-secondary)", borderBottom: "1px solid var(--border-color)" }}>
                {["Setup", "Trades", "Win %", "Net PNL", "Avg Win", "Avg Loss", "R:R", "Best", "Worst", "L/S"].map(h => (
                  <th key={h} style={{ padding: "9px 12px", fontWeight: 500, textAlign: h === "Net PNL" || h === "Best" || h === "Worst" ? "right" : "left", color: "var(--text-secondary)", whiteSpace: "nowrap", fontSize: "12px" }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {setups.map((s, i) => (
                <tr key={s.setup} className="notion-table-row" style={{ borderBottom: i < setups.length - 1 ? "1px solid var(--border-color)" : "none" }}>
                  <td style={{ padding: "9px 12px", fontWeight: 600, maxWidth: "120px", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{s.setup}</td>
        ...
        {/* Bar chart — always visible, good on mobile */}
        <div style={{ marginTop: "16px", display: "flex", flexDirection: "column", gap: "10px" }}>
          {setups.map(s => (
            <div key={s.setup}>
              <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "4px" }}>
                <span style={{ fontSize: "13px", fontWeight: 600, color: "var(--text-primary)" }}>{s.setup}</span>
                <div style={{ display: "flex", gap: "12px", alignItems: "center" }}>
                  <span style={{ fontSize: "12px", color: "var(--text-secondary)" }}>{s.total} trades · {s.winRate.toFixed(0)}% WR</span>
